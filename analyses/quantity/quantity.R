# Creates the file `results/quantity_table.csv` by evaluating GAF file quantities and information from `data/n_genes_per_genome.csv`
library(tools)
...
possibly_read_gaf <- function(gaf_file) {
  ...
  if(is.character(gaf_file))
    return(read.csv(gaf_file, header=T, skip=1, sep="\t"))
...
csv = data.frame(
  ...
  genes_annotated_p = 0,
  genes_annotated_a = 0,
  # Proportion (percentage) of genes annotated
  p_genes_annotated_c = 0,
  p_genes_annotated_f = 0,
  p_genes_annotated_p = 0,
  p_genes_annotated_a = 0,
...
for(genome_path in genomes) {
  ...
  for(gaf_file in list.files(genome_path, pattern = ".gaf", full.names = T)) {
    ...
    csv <- rbind(csv, data.frame(
      ...
      p_genes_annotated_c = genes_annotated[["C"]]/n_genes_per_genome[n_genes_per_genome$genome == genome_name,"n_genes"]*100,
      p_genes_annotated_f = genes_annotated[["F"]]/n_genes_per_genome[n_genes_per_genome$genome == genome_name,"n_genes"]*100,
      p_genes_annotated_p = genes_annotated[["P"]]/n_genes_per_genome[n_genes_per_genome$genome == genome_name,"n_genes"]*100,
      p_genes_annotated_a = genes_annotated[["A"]]/n_genes_per_genome[n_genes_per_genome$genome == genome_name,"n_genes"]*100,
