# Creates the file `data/mocks/results/quantity_table.csv` by evaluating GAF file quantities and information from `data/mocks/raw/n_genes_per_genome.csv`
library(tools)
...
# Pass the path to a GAF file and it will return how many genes have at least one annotation in each aspect (C,F,P) or any (A)
...
n_genes_per_genome = read.csv("data/mocks/raw/n_genes_per_genome.csv", header = T)
genomes = list.dirs("data/mocks/raw/go_annotation_sets", recursive = F)
...
write.csv(csv, 'data/mocks/results/quantity_table.csv', row.names = F)
